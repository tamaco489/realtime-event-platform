...
│   │   ├── presignup/main.go    # Pre Sign-up Lambda エントリポイント
│   │   └── authorizer/main.go   # AppSync Authorizer Lambda エントリポイント
│   ├── internal/
│   │   ├── handler/
│   │   │   ├── api/             # REST ハンドラー → producer
│   │   │   ├── event/           # SQS ハンドラー → notifier
│   │   │   ├── presignup/       # Cognito Pre Sign-up トリガー → テナント検証
│   │   │   └── authorizer/      # AppSync Lambda Authorizer → JWT 検証
│   │   └── library/
│   │       ├── config/          # 環境設定
